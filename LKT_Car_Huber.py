...
def lucas_kanade_tracker(img, tmp, rect, p_prev, flag):
    ...
    while cost_function > threshold:
        ...
        if np.linalg.norm(error)<threshold:
            loss=0.5*(np.linalg.norm(error)**2)
        else:
            loss=(threshold*np.linalg.norm(error))-(0.5*threshold**2)
...
vid_output = cv.VideoWriter('lkt_car_brightness.avi', cv.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (images[0].shape[1], images[0].shape[0]))
