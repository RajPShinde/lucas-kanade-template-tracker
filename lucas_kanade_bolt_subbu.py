...
def lucas_kanade_tracker(ref_points, template_image, next_image):
    ...
    while cost_function > threshold: 
        ...
        # Compute error image
        err_image = template_image.flatten().astype(np.int) - warp_image.flatten().astype(np.int)
        # err_image = err.reshape(-1, 1)

        Ix = cv.Sobel(next_image, cv.CV_64F, dx=1, dy=0, ksize=1) # ksize -1 for scharr
        Iy = cv.Sobel(next_image, cv.CV_64F, dx=0, dy=1, ksize=1)
        # abs_ix = np.absolute(Ix)
        # abs_iy = np.absolute(Iy)

        # Ix = np.uint8(abs_ix)
        # Iy = np.uint8(abs_iy)
        grad_x = get_region_of_interest(cv.warpAffine(Ix, W, (Ix.shape[1], Ix.shape[0]), flags=cv.INTER_AREA + cv.WARP_INVERSE_MAP), ref_points)
        grad_y = get_region_of_interest(cv.warpAffine(Iy, W, (Iy.shape[1], Iy.shape[0]), flags=cv.INTER_AREA + cv.WARP_INVERSE_MAP), ref_points)
        ...
        p_change = np.dot(np.linalg.pinv(H), np.dot(steepestImg.T, err_image)) 
        ...
        # p[0] = p[0] + p0 + p_prev[0]*p0 + p_prev[2]*p1
        # p[1] = p[1] + p1 + p_prev[1]*p0 + p_prev[3]*p1
        # p[2] = p[2] + p2 + p_prev[0]*p2 + p_prev[2]*p3
        # p[3] = p_prev[3] + p3 + p_prev[1]*p2 + p_prev[3]*p3
        # p[4] = p_prev[4] + p4 + p_prev[0]*p4 + p_prev[2]*p5
        # p[5] = p_prev[5] + p5 + p_prev[1]*p4 + p_prev[3]*p5
