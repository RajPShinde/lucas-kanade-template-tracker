...
out = cv.VideoWriter('lkt_on_bolt_code_best.avi', cv.VideoWriter_fourcc('M','J','P','G'), 15, (480, 270))
...
def lucas_kanade_tracker(ref_points, template_image, next_image, p_prev):
    ...
    Ix = cv.Sobel(next_image, cv.CV_16S, dx=1, dy=0, ksize=7) # ksize -1 for scharr
    ...
    while np.linalg.norm(dp) > threshold: #for it in range(max_iterations):
        ...
        # while(1):
        #     cv.imshow('GradX', gx_img_w)
        #     cv.imshow('GradY', gy_img_w)
        #     key = cv.waitKey(1)
        #     if key == 27:
        #         break
